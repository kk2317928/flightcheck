...
PY
  docker compose -f compose.worker.yaml --env-file .env.worker.example config >"$config_file"
  CONFIG_FILE="$config_file" python3 <<'PY'
import os, yaml
with open(os.environ['CONFIG_FILE'], encoding='utf-8') as handle:
    config = yaml.safe_load(handle)
assert set(config['services']) == {'migrate', 'worker'}
worker = config['services']['worker']
assert worker['restart'] == 'unless-stopped'
assert worker['depends_on']['migrate']['condition'] == 'service_completed_successfully'
assert worker['read_only'] is True
assert any(item['type'] == 'volume' for item in worker['volumes'])
PY
else
  python3 <<'PY'
import yaml
...
PY
  python3 <<'PY'
import yaml
with open('compose.worker.yaml', encoding='utf-8') as handle:
    config = yaml.safe_load(handle)
assert set(config['services']) == {'migrate', 'worker'}
worker = config['services']['worker']
assert worker['restart'] == 'unless-stopped'
assert worker['depends_on']['migrate']['condition'] == 'service_completed_successfully'
assert worker['read_only'] is True
assert worker['volumes'][0].split(':', 1)[0] in config['volumes']
...
if grep -En '(password|secret|token):[[:space:]]+[^$]' compose.worker.yaml; then
  echo 'compose.worker.yaml contains an embedded secret-like value' >&2
  exit 1
fi
